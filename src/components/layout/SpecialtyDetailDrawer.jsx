      ...
      <div className={`detail-sheet-container ${activeSpecialtyDetailId ? 'open' : ''}`}>
        ...
        <div className="detail-sheet-body">
          ...
          <div className="detail-sheet-meta-card">
            <span className="material-symbols-outlined" style={{ fontSize: '24px', color: 'var(--md-sys-color-secondary)' }}>assignment</span>
            <div>
            ...
            <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {activeService.treatments?.map((tr, idx) => (
                <li key={idx} style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '0.95rem' }}>
                  <span className="material-symbols-outlined" style={{ color: 'var(--md-sys-color-secondary)', fontSize: '18px', fontWeight: 'bold' }}>check_circle</span>
                  <span>{tr}</span>
                </li>
              ))}
            </ul>
          ...
          {activeService.preparations && (
            <div className="detail-sheet-instruction-card">
              <div style={{ display: 'flex', gap: '10px' }}>
                <span className="material-symbols-outlined" style={{ fontSize: '24px', color: 'var(--md-sys-color-secondary)' }}>info</span>
                <div>
